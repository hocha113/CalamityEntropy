...
//   被吞噬的星球  圆盘 + 边缘暗化 + 伪 3D 表面(s2)缓慢自转 + 冷紫背光;uErosion 按噪声阈值挖空成虚空黑,挖口一圈热边,下半球先被啃
//   封锁力场    六边形距离场细边线,以 uBossUv 为中心高斯亮化(乘核心亮度),拍点整面亮 + 从本体扩散的冲击环,主炮蓄力时本体周围逐格填实
...
float4 PixelFunc(float2 uv : TEXCOORD0) : COLOR0
{
    ...
    // ---- 被吞噬的星球 ----
    float2 pc = float2(uPlanetCenter.x * asp, lerp(uPlanetCenter.y, 1.0 - uPlanetCenter.y, uFlip)) - uParallax * uPlanetParallax;
    float2 pp = (sp - pc) / max(uPlanetRadius, 0.01);
    float d = length(pp);
    float disk = 1.0 - smoothstep(0.985, 1.0, d);
    float z = sqrt(saturate(1.0 - d * d));
    // 伪 3D:越靠边表面坐标拉得越开,读成球面;+0.3 免得边缘除零
    float2 sph = pp / (z + 0.3);
    float2 suv = sph * 0.32 + float2(uTime * 0.006, 0.0);
    float terrain = tex2D(uImage2, frac(suv)).r * 0.65 + tex2D(uImage2, frac(suv * 2.3 + 0.5)).r * 0.35;
    ...
    float3 albedo = lerp(float3(0.05, 0.035, 0.08), float3(0.20, 0.15, 0.28), terrain);
    float3 planet = albedo * (0.3 + 0.7 * ndl) * limb + uColorPlanetRim * pow(1.0 - z, 3.0) * 0.4;
    // 侵蚀:噪声场超过阈值处挖空;下半球(朝战场)加权先被啃;挖口一圈热边
    float2 euv = sph * 0.55 + 0.13;
    ...
    float3 eatenCol = float3(0.01, 0.0, 0.03) + uColorErosion * (0.05 + 0.10 * en);
    float3 planetCol = lerp(planet, eatenCol, eaten) + uColorErosion * edge * 0.9;
    col = lerp(col, planetCol, disk);
    // 星球外圈微晕,随侵蚀加深
    float halo = exp(-max(d - 1.0, 0.0) * 5.0) * (1.0 - disk);
    col += uColorErosion * halo * (0.05 + 0.10 * uErosion);
    ...
    float ring = exp(-ringD * ringD) * ringOn * ringFade;
    float gridI = uGridAlpha * breathe + bossMask * 0.35 + uFlash * 0.45 + ring * 0.9 + uCharge * 0.2;
