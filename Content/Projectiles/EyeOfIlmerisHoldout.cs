...
{
    public class EyeOfIlmerisHoldout : ModProjectile
    {
        ...
        public override void AI() {
            ...
            if (Projectile.ai[0]++ > 16 && Projectile.ai[0] % 10 == 0) {
                if (owner.CheckMana(5, true)) {
                    if (Main.myPlayer == Projectile.owner) {
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity.RotatedByRandom(0.35f) * 0.6f, ModContent.ProjectileType<IlmerisWaterBullet>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                    }
                    if (!Main.dedServ) {
