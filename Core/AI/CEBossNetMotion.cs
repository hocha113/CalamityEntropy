...
namespace CalamityEntropy.Core.AI
{
    ...
    /// </para>
    /// <para>
    /// 另一半是计时。VaultState.Timer/Counter 是纯本地量,客户端换态时新实例从 0 起跑,
    /// 同一套运动数学便从不同起点推进:锁向帧、发射拍、刹车拍全都错开,那是真失步而非抖动。
    /// 所以权威端把当前状态的计时随快照同包送出(SendExtraAI 与位置/速度原子到达),
    /// 客户端带容差收养:只差一两帧属网络抖动常态,硬对齐反而会让 Timer == X 型一次性拍被跳过或重放。
    /// </para>
    /// </summary>
    public sealed class CEBossNetMotion
    {
        ...
        /// <summary>
        /// 蓄势颤抖等抖动只走绘制层:原版把 NPC 画在 position + netOffset,
        /// 而本件每帧开头已把 netOffset 清零,所以 AI 期写进去的量只影响本帧贴图,位置与判定分毫不动。
